...
import { Target, Eye, Globe, Award, Users, TrendingUp, ArrowRight, Linkedin } from 'lucide-react'
...
const values = [
  { icon: Target, title: 'Accuracy First', body: 'Every data point is verified through primary research, expert interviews, and cross-referenced with trade databases.' },
  { icon: Globe, title: 'Global Perspective', body: 'Our research covers 85+ countries with local nuance that global databases often miss.' },
  { icon: Award, title: 'Actionable Insight', body: 'We do not deliver data for its own sake. Every report includes strategic recommendations you can act on.' },
  { icon: Users, title: 'Client Obsessed', body: 'Our research agenda is driven by client feedback. We research markets you need - not markets we find convenient.' },
]

const milestones = [
  { year: '2019', event: 'Integer Market founded with focus on nutraceutical and pharma research' },
  { year: '2021', event: 'Expanded to 8 industries; crossed 500 research report milestone' },
  { year: '2023', event: 'Launched digital platform with instant download; 2,000+ clients globally' },
  { year: '2024', event: 'Expanded to 16 industries; 1,200+ reports in catalog' },
  { year: '2026', event: 'Launched AI-assisted research methodology; 5,000+ clients in 85+ countries' },
]
...
const researchers = [
  { name: 'Dr. Aditya Dhadwe', role: 'Consumer Health (MBBS)', photo: adityaPhoto },
    ...
    <div className="min-h-screen bg-surface">
      ...
      <div className="bg-white border-b border-slate-100 pt-24 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <ScrollReveal>
            <Breadcrumb items={[{ label: 'About Us' }]} className="mb-5 justify-center" />
            <Badge variant="primary" className="mb-5">Our Story</Badge>
            <h1 className="text-5xl font-extrabold text-slate-900 mb-5">Built for Decision-Makers</h1>
            <p className="text-slate-500 text-lg max-w-2xl mx-auto">
              Integer Market was founded on a simple belief: strategic decisions should be backed by rigorous, accessible market intelligence - not expensive consultancy retainers.
            </p>
          </ScrollReveal>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-20">
          {[
            { icon: Target, title: 'Our Mission', body: 'To democratize access to world-class market intelligence. We believe that actionable research should be available to every business - from growing startups to Fortune 500 corporations - at a price that makes sense.' },
            { icon: Eye, title: 'Our Vision', body: 'To become the most trusted source of market intelligence for the global pharma, nutraceutical, and specialty chemical industries - powering decisions that shape the future of health and wellness globally.' },
        ...
        <ScrollReveal className="mb-20">
          ...
          >
            {[
              { val: '1,200+', label: 'Reports Published', icon: TrendingUp },
              { val: '16', label: 'Industries Covered', icon: Target },
              { val: '85+', label: 'Countries Tracked', icon: Globe },
              { val: '5,000+', label: 'Global Clients', icon: Users },
            ].map(({ val, label, icon: Icon }) => (
              <motion.div key={label} variants={scaleIn} className="bg-white border border-slate-100 rounded-2xl p-6 text-center">
                <Icon size={20} className="text-primary mx-auto mb-2" aria-hidden="true" />
        ...
        >
          {values.map(({ icon: Icon, title, body }) => (
            <motion.div key={title} variants={fadeInUp} className="bg-white border border-slate-100 rounded-2xl p-6 flex gap-5">
        ...
        <ScrollReveal className="text-center mb-10">
          <span className="inline-block text-xs font-semibold text-primary uppercase tracking-widest mb-3">The People Behind It</span>
          <h2 className="text-3xl font-bold text-slate-900">Leadership Team</h2>
          <p className="text-slate-500 mt-2 max-w-xl mx-auto">Domain experts and operators who built Integer Market with a single mission: better market intelligence for everyone.</p>
        </ScrollReveal>
        ...
        >
          {leadership.map((person, i) => (
            ...
            >
              ...
              <h3 className="text-base font-bold text-slate-900 mb-0.5">{person.name}</h3>
              <p className="text-xs font-semibold text-primary mb-3">{person.title}</p>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-8 lg:p-12 mb-20">
          ...
          >
            {researchers.map((r) => (
              <motion.div key={r.name} variants={fadeInUp} className="flex flex-col items-center text-center">
        ...
        <ScrollReveal>
          <div className="bg-gradient-to-br from-primary to-primary-dark rounded-2xl p-10 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/70 mb-6">Explore our research catalog or reach out to our team.</p>
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <Link href="/report" className="flex items-center gap-2 px-7 py-3.5 bg-white text-primary font-semibold rounded-xl hover:bg-slate-50 transition-colors shadow-md">
                  Browse Reports <ArrowRight size={16} aria-hidden="true" />
                </Link>
                <Link href="/contact" className="flex items-center gap-2 px-7 py-3.5 border border-white/30 text-white font-semibold rounded-xl hover:bg-white/10 transition-colors">
                  Contact Us
                </Link>
              </div>
            </div>
